 ...
 * @LastEditTime: 2020-10-13 14:49:32
 */
/* eslint-disable */
...
class CommonUtils {
    ...
    */
     ...
     * @param {Array} arr
    */
    async getDicAllDataList (arr, _this, isApi) {
        for (const i in arr) {
          const item = arr[i];
          await this.getDataList(item, _this, isApi);
        }
      }
      getDataList (item, _this, isApi) {
        const index = config.dataList.findIndex(v => v.code === item.dicCode);
        if (index < 0) return;
        const id = config.dataList[index].id;
        this._getDicItemValues(id, item.dicCode, item.propCode, _this, isApi);
      }
      _getDicItemValues (id, dicCode, propCode, _this, isApi) {
        const url = isApi ? 'publicApi/getDicItemApiValues' : 'publicApi/getDicItemValues';
        store.dispatch(url, {dicId: id}).then(res => {
      ...
      setDicConfig(propCode, res, _this) {
        ...
        // 编辑页面多个主表
        if (_this.PageConfig.mainFormDetailConfig) {
            const index = _this.PageConfig.mainFormDetailConfig.formList.findIndex(v => v.prop === propCode);
            if (index >= 0) {
                _this.PageConfig.mainFormDetailConfig.formList[index].selectList = res;
            }
        }
        // 查询页面 搜索赋值
        if (_this.PageConfig.searchControls) {
        ...
        if (_this.PageConfig.subTableConfig) {
            for (const item of _this.PageConfig.subTableMatch) {
                ...
                if (index >= 0) {
                    tableList.slaveColumns[index].selectList = res;
                }
            }
        }
      }
